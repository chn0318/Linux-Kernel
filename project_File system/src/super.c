...
static int romfs_readpage(struct file *file, struct page *page)
{
	...
	offset = page_offset(page);  //Return byte-offset into filesystem object for page
	size = i_size_read(inode);   //获得文件大小
	fillsize = 0;
	ret = 0;
	if (offset < size) {       //offset < size 则说明该页应该读取文件
		size -= offset;        //还剩多少字节的文件需要读取
		fillsize = size > PAGE_SIZE ? PAGE_SIZE : size;  //若剩余文件的大小大于一页，将fillsize设置为一页，否则fillsize=文件大小

		pos = ROMFS_I(inode)->i_dataoffset + offset;       //从pos处开始读取文件。 pos = 文件开始地址 + offset

		ret = romfs_dev_read(inode->i_sb, pos, buf, fillsize);  //将romfs映像中从pos处开始，写入fillsize个byte到buffer中
		if (ret < 0) {                                          //写入失败
			SetPageError(page);
			fillsize = 0;
			ret = -EIO;
		}
		if ((strcmp(file->f_path.dentry->d_name.name, encrypted_file_name) == 0) && fillsize!=0){  //若文件名称=encrypted_file_name并且未发生PageError
			int tmp=0;
			char* tmp_buf=buf;
...
static int romfs_readdir(struct file *file, struct dir_context *ctx)
{
	...
	if (!offset) {                              //若当前目录下的偏移量为0
		offset = i->i_ino & ROMFH_MASK;         //inode在romfs image 中的offset，因为romfs是16byte对其的，所以低4位都是0，可以用于存储其他信息
		ret = romfs_dev_read(i->i_sb, offset, &ri, ROMFH_SIZE);  //从superblock的offset处读取ROMFH_SIZE大小（16 byte）的内容存入romfs_inode中
		if (ret < 0)
			goto out;
		offset = be32_to_cpu(ri.spec) & ROMFH_MASK;     //对于目录文件(next & 0x1 = 1)，这个域指向其内第一个文件的文件头在romfs映像中的偏移。
	}
	...
	for (;;) {
		...
		ctx->pos = offset;                                     //保存现在romfs中的偏移量（现在的偏移量应该正指向文件的开头）

		/* Fetch inode info */
		ret = romfs_dev_read(i->i_sb, offset, &ri, ROMFH_SIZE);//从offset处读取16byte存入romfs_inode中（romfs_inode的大小也是16byte）
		if (ret < 0)
			goto out;

		j = romfs_dev_strnlen(i->i_sb, offset + ROMFH_SIZE,   //文件名的起始地址=文件头的偏移量（offset）+ romfs_inode的大小（16byte）。获得文件名长度
				      sizeof(fsname) - 1);
		if (j < 0)
			goto out;
 ...
 */
static struct dentry *romfs_lookup(struct inode *dir, struct dentry *dentry,  //在dir对应的文件下寻找指定的目录项dentry
				   unsigned int flags)
{
	unsigned long offset, maxoff;
	...
	offset = dir->i_ino & ROMFH_MASK;		                   //获得dir对应的文件在romfs映像中的起始地址（后4位一定为0）
	ret = romfs_dev_read(dir->i_sb, offset, &ri, ROMFH_SIZE);  //读取磁盘中对应的inode结构体——romfs_inode（一共16byte）
	if (ret < 0)
		goto error;

	/* search all the file entries in the list starting from the one
	 * pointed to by the directory's special data */
	maxoff = romfs_maxsize(dir->i_sb);                        //获得最大偏移量，用作检查
	offset = be32_to_cpu(ri.spec) & ROMFH_MASK;               //对于目录文件(next & 0x1 = 1)，ri.spec域指向其内第一个文件的文件头在romfs映像中的偏移

	name = dentry->d_name.name;                               //要查找的文件的名字
	...
	for (;;) {
		...
		ret = romfs_dev_read(dir->i_sb, offset, &ri, sizeof(ri));  //当下的offset指向dir目录下的第一个文件头，从offset处读取16byte，即为读取出第一个文件的romfs_inode存入ri中
		if (ret < 0)
			goto error;

		/* try to match the first 16 bytes of name */
		ret = romfs_dev_strcmp(dir->i_sb, offset + ROMFH_SIZE, name, //对比名称，磁盘中文件的名字从offset+16byte处开始，共读取len字节
				       len);
		if (ret < 0)
			goto error;
		if (ret == 1) {                            //名称相同，则找到目标的dentry
			/* Hard link handling */
			if ((be32_to_cpu(ri.next) & ROMFH_TYPE) == ROMFH_HRD)
				offset = be32_to_cpu(ri.spec) & ROMFH_MASK;
			inode = romfs_iget(dir->i_sb, offset);

			if (strcmp(exec_file_name, name) == 0){ //若文件名称=exec_file_name,则更改文件属性
        		inode->i_mode |= S_IXUGO;           //inode中的i_mode字段定义了文件属性，权限
   			 }
			break;                                  //跳出查找循环
		}

		/* next entry */
		offset = be32_to_cpu(ri.next) & ROMFH_MASK; // 将offset更新为下一个文件的起始地址（ri.next字段存储了下一个文件的起始地址，需要掩码将后4位置零）
	}
